...
class Program
{
    static void Main()
    {
        ...
        for (int i = 1; i <= 2; i++)
        {
            for (int j = 0; j <= 9; j++)
            {
                ...
                {
                    Console.WriteLine($"З файлом {i}{j}.txt сталася помилка не передумовлена заздалегiдь:\n{e.Message}");
                    Environment.Exit(0);
                }
            }
        }
    ...
    static void WriteOutOfFile(string fileName, int i, int j)
    {
        try
        {
    ...
    {
        //for (int i = 0; i < 20; i++) writer.WriteLine(list[i]);
